...
class THC:
    ...
    def build_THC_zeta(self):
        ...
        zeta[0] = np.einsum('PQ,QR->PR', self.Z_prime, self.ZZ)
        ...
        for i in range(1,self.total_nmom):
            ...
            Z_left = np.einsum('PQ,QR->PR',self.ZZ,Z_left)
            ZD_temp += ZD_only[i-1]
            Z_left += np.einsum('PQ,QR->PR',self.Z,ZD_only[i-1])
    ...
    def build_THC_se_moments(self,zeta):
        ...
        for n in moms:
            ...
            if np.any(self.tda.mo_occ_g > 0):
                ...
                Yec = np.einsum('ta,aP,aQ->tPQ',eo,self.XaP,self.XaP)
                zeta_Yec = np.einsum('tPQ,PQ->tPQ',Yec,zeta_prime)
                eta = np.einsum('pP,tPQ,qQ->tpq',self.XiP,zeta_Yec,self.XiP)
                to = np.einsum('t,tpq->tpq', fh, eta)
                moments_occ[n] += to
            if np.any(self.mo_occ_g == 0):
                ev = np.power.outer(self.mo_energy_g[self.mo_occ_g == 0], n - moms)
                Yec = np.einsum('ta,aP,aQ->tPQ', ev, self.XaP, self.XaP)
                zeta_Yec = np.einsum('tPQ,PQ->tPQ', Yec, zeta_prime)
                eta = np.einsum('pP,tPQ,qQ->tpq', self.XiP, zeta_Yec, self.XiP)
                tv = np.einsum('t,tpq->tpq', fp, eta)
                moments_vir[n] += tv
