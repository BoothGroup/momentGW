...
def hash_array(array, tol=KPT_DIFF_TOL):
    """
    Get a hashable representation of an array up to a given tol.
    ...
    array : np.ndarray
        Array.
    tol : float, optional
        Threshold for the hashable representation.  Default is equal
        to `pyscf.pbc.lib.kpts_helper.KPT_DIFF_TOL`.

    Returns
    -------
    array_round : tuple
        Hashable representation of the array.
    """

    array_round = np.rint(np.asarray(array) / tol).astype(int)
    return tuple(array_round.ravel())


def make_auxcell(with_df, auxbasis=None, drop_eta=None):
    """
    ...
    auxcell = addons.make_auxmol(with_df.cell, auxbasis)
    ...
    for ib in range(len(auxcell._bas)):
        ...
        if nprim > 0:
            ...
            r = _estimate_rcut(exps, l, np.abs(coeffs).max(axis=1), with_df.cell.precision)
            rcut.append(r.max())
...
def make_chgcell(with_df, smooth_eta, rcut=15.0):
    """
    ...
    chgcell = copy.copy(with_df.auxcell)
    chg_bas = []
    chg_env = [smooth_eta]
    ...
    chgcell._atm = with_df.auxcell._atm
    chgcell._bas = np.asarray(chg_bas, dtype=np.int32).reshape(-1, gto.BAS_SLOTS)
    chgcell._env = np.hstack((with_df.auxcell._env, chg_env))

    # _estimate_rcut is too tight for the model charge
    chgcell.rcut = np.sqrt(np.log(4 * np.pi * rcut**2 / with_df.auxcell.precision) / smooth_eta)
...
def find_conserving_qpts(cell, qpts, qpt, tol=KPT_DIFF_TOL):
    """
    Search a list of q-points for those that conserve momentum as
    Search which (qpts+qpt) satisfies momentum conservation.
    ...
    qpt : np.ndarray
        Other q-point.
    tol : float, optional
        Threshold for equality.  Default is equal to
        `pyscf.pbc.lib.kpts_helper.KPT_DIFF_TOL`.
    ...
    mask = np.sum(np.abs(dif - dif_int), axis=0) < tol
    ids = np.where(mask)[0]

    return ids


def _get_2c2e(with_df, uniq_qpts):
    """
...
def _get_3c2e(with_df, kpt_pairs):
    """
    ...
    log.timer("3c2e", *cput0)

    return int3c2e


def _get_j2c(with_df, int2c2e, uniq_qpts):
    """
    ...
    for q, qpt in enumerate(uniq_qpts):
        cput1 = (logger.process_clock(), logger.perf_counter())

        G_chg = ft_ao.ft_ao(with_df.fused_cell, Gv, b=b, gxyz=gxyz, Gvbase=Gvbase, kpt=qpt).T
        G_aux = G_chg[naux:] * with_df.weighted_coulG(qpt)
        ...
        j2c[q] = with_df.fuse(with_df.fuse(j2c[q]).T).T

        del G_chg, G_aux

        log.timer_debug1("j2c [qpt %d] part" % q, *cput1)
...
def _cholesky_decomposed_metric(with_df, j2c):
    """
    Get a function which applies the Cholesky decomposed j2c for a
    single q-point.
    ...
    if j2c_chol is None:
        ...
        log.info("DF metric linear dependency detected")
...
def _get_j3c(with_df, j2c, int3c2e, uniq_qpts, uniq_inverse_dict, kpt_pairs, out=None):
    """
    ...
    for q in mpi_helper.nrange(len(uniq_qpts)):
        cput1 = (logger.process_clock(), logger.perf_counter())
        log.debug("Constructing j3c [qpt %d].", q)

        qpt = uniq_qpts[q]
        adapted_pair_idx = uniq_inverse_dict[q]
        adapted_kpts = kpt_pairs[:, 1][adapted_pair_idx]

        log.debug("qpt = %s", qpt)
        log.debug("adapted_pair_idx = %s", adapted_pair_idx)

        # Prepare cholesky decomposition of j2c
        if j2c is not None:
            pair_idx = find_conserving_qpts(cell, uniq_qpts, -qpt)[0]
            j2c_chol = _cholesky_decomposed_metric(with_df, j2c[pair_idx])
            log.debug("pair_idx = %s", pair_idx)
        ...
        for kji, ji in enumerate(adapted_pair_idx):
            # Eq. 31 first term:
            v = int3c2e[ji]
...
def _make_j3c(with_df, kpt_pairs):
    ...
    int2c2e = _get_2c2e(with_df, uniq_qpts)

    # Get the 3c2e interaction:
    int3c2e = _get_3c2e(with_df, kpt_pairs)

    # Get j2c:
    j2c = _get_j2c(with_df, int2c2e, uniq_qpts)

    # Get j3c:
    j3c = _get_j3c(with_df, j2c, int3c2e, uniq_qpts, uniq_inverse_dict, kpt_pairs)
...
class GDF(df.GDF):
    ...
    def build(self, j_only=None, with_j3c=True):
        ...
        self.auxcell = make_auxcell(self, auxbasis=self.auxbasis, drop_eta=self.exp_to_discard)
        self.chgcell = make_chgcell(self, self.eta, rcut=self.rcut_smooth)
        self.fused_cell, self.fuse = fuse_auxcell(self)
        ...
        for k, kpt in enumerate(kpt_pairs):
            val = hash_array(kpt)
            self.kptij_hash[val].append(k)

        if with_j3c:
            self._cderi = self._make_j3c(kpt_pairs)
    ...
    def sr_loop(self, kpti_kptj=np.zeros((2, 3)), max_memory=None, compact=True, blksize=None):
        ...
        if self._cderi is None:
            self.build()
        kpti, kptj = kpti_kptj
        ki = self.kpt_hash[hash_array(kpti)][0]
        kj = self.kpt_hash[hash_array(kptj)][0]
    ...
    def get_3c_eri(self, kpts, compact=COMPACT):
        ...
        ki, kj = (self.kpt_hash[hash_array(kpt)][0] for kpt in kptij)
